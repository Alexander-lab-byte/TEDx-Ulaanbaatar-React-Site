...
const speakers = [
  ...
  ['Local Teacher Speaker', '12-minute talk on encouraging curiosity over rote memorization.'],
  ['Student Speaker', '6-minute talk on how high school volunteering transforms communities.'],
  ['External Speaker', '15-minute talk connecting Mongolian heritage with Gen-Z innovation.'],
  ['Local Teacher Speaker', '12-minute talk mentoring young researchers for regional impact.'],
  ['External Speaker', '15-minute talk on taking bold entrepreneurial steps in youth.'],
  ['Student Speaker', '6-minute talk on why embracing creative risk shapes the future.'],
];
...
function Team() {
  ...
  return <><Nav /><main className="page-main"><section className="team-hero"><div className="wrap reveal"><div className="eyebrow">{tx(b('Behind the Stage', 'Тайзны ард'))}</div><h1>{tx(b('Meet the visionaries.', 'Зохион байгуулах багтай танилц.'))}</h1><p>{tx(b('The dedicated team working behind the scenes to make this event happen.', 'Энэхүү эвэнтэд зориулан тайзны ард ажиллаж буй манай баг хамт олон.'))}</p><div className="stats"><div className="stat"><strong>?</strong><span>{tx(b('Total members', 'Нийт гишүүд'))}</span></div><div className="stat"><strong>04</strong><span>{tx(b('Departments', 'Албадууд'))}</span></div><div className="stat"><strong>100%</strong><span>{tx(b('Volunteer driven', 'Сайн дурын баг'))}</span></div><div className="stat"><strong>2026</strong><span>{tx(b('Edition team', '2026 оны баг'))}</span></div></div></div></section><div className="filter-bar"><div className="wrap"><div className="filters"><button className={`filter ${filter === 'all' ? 'active' : ''}`} onClick={() => setFilter('all')} data-testid="button-filter-all">{tx(b('All Departments', 'Бүх алба'))}</button>{departments.map(([id, label]) => <button key={id} className={`filter ${filter === id ? 'active' : ''}`} onClick={() => setFilter(id)} data-testid={`button-filter-${id}`}>{tx(label)}</button>)}</div></div></div><div className="wrap team-content">{departments.filter(([id]) => filter === 'all' || id === filter).map(([id, label]) => { const members = visible.filter((member) => member.dept === id); return <section className="dept" key={id}><div className="dept-heading"><h2>{tx(label)}</h2><i /><span>{members.length} {tx(b('Members', 'Гишүүн'))}</span></div><div className="team-grid">{members.map((member, index) => <article className="member" key={`${id}-${index}`}><div className="member-visual"><span>{member.name === '?' ? '?' : member.name.slice(0, 1)}</span></div><div className="member-info"><div className="member-role">{tx(member.role)}</div><h3>{member.name}</h3><p>{tx(member.bio)}</p></div></article>)}</div></section>; })}</div></main><Footer /></>;
}
